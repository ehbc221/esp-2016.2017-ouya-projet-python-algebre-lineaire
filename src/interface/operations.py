...
def operation_produit(mat1, mat2, nb1, nb2, nb3, nb4):
    if nb2 != nb3:
        messagebox.showerror("Erreur", "Produit Impossible")
        return -1
    else:
...
def operation_somme(mat1, mat2, nb1, nb2, nb3, nb4):
    if (nb1 != nb3) | (nb2 != nb4):
        messagebox.showerror("Erreur", "Somme Impossible")
        return -1
    else:
        ...
        for i in range(nb1):
            for j in range(nb2):
                somme[i][j] = Button(root_somme,
                                        text=float(mat1[i][j].get()) + float(mat2[i][j].get()),
                                        width=4,
                                        height=1,
                                        relief="groove",
                                        font="/font/myfont 9 ",
...
def operation_stockmorse(mat, n):
    ...
    for i in range(n):
        for j in range(n):
            if float(mat[i][j].get()) != 0.0:
                indiceligne[x] = Button(root_morse,
                                        text=i,
                                        width=4,
                                        height=1,
                                        relief="groove",
                ...
                indicecolonne[x] = Button(root_morse,
                                            text=j,
                                            width=4,
                                            height=1,
                                            relief="groove",
                ...
                elmtnonnul[x] = Button(root_morse,
                                        text=mat[i][j].get(),
                                        width=4,
                                        height=1,
                                        relief="groove",
                ...
                x += 1
    canvas.create_text(50, (100 * n - 50) / 2 - 20,
                        text="i",
                        font="/font/myfont 9 ",
                       fill="black"
                    )
    canvas.create_text(50, (100 * n - 50) / 2,
                        text="j",
                        font="/font/myfont 9 ",
                        fill="black"
                    )
...
def operation_inverse(mat, n):
    ...
    if inversible == 1:  # Affichage de la matrice inverse
        ...
        while i < n:
            j = n
            while j < 2 * n:
                inv[i][j] = Button(root_inverse, text=round(newmat[i][j], 2),
                                    relief="groove",
                                    width=4, height=1,
                                    font="/font/myfont 9 ",
                                    bg="#eee")
                canvas.create_window((200 * n - 100) / 2 - 200 + 50 * j, (100 * n - 50) / 2 - 30 + 40 * i, window=inv[i][j])
                j += 1
            i += 1
        canvas.create_text((200 * n - 100) / 2 - 100, (100 * n - 50) / 2 + 10,
                            text="Inv(A)= ",
                            font="/font/myfont 12", fill="black"
                        )
        canvas.pack()
    else:
        messagebox.showinfo(":-/", "La matrice n'est pas inversible")
...
def operation_determinant(mat1, n1):
    (root_det, canvas) = root_operation(n1, "Déterminant de la matrice")
    ...
    for i in range(n, 0, -1):
        for j in range(0, i):
            if mat[i][j + 1] != 0.0:
                ...
                sign *= -1
    determinant = Button(root_det,
                            text=round(sign * proddiag(mat, n1), 2),
                            relief="groove",
                            width=6,
                            height=1,
                            font="/font/myfont 15 ",
    ...
    canvas.create_text((200 * n1 - 100) / 2 - 90, (100 * n1 - 50) / 2 - 20,
                        text="det(M) = ",
                        font="/font/myfont 12 ",
                        fill="black"
                    )
...
def operation_transposee(mat1, n):
    (root_trans, canvas) = root_operation(n, "Transposée de la matrice")
    ...
    for a in range(n):
        for b in range(n):
            ...
            canvas.create_window((200 * n - 100) / 2 - 50 + 45 * b, (100 * n - 50) / 2 + 50 * a,
                                 window=trans[a][b])
    canvas.create_text((200 * n - 100) / 2 - 150, (100 * n - 50) / 2 + 50,
                        text="            t(A)= ",
                        fill="black",
                        font="/font/myfont 12 "
                    )
...
def operation_gauss(mat, vect, n):
    ...
    for k in range(n):
        ...
        while i < n:
            ...
            i += 1
        if valmin == 0:
            messagebox.showinfo("Erreur", "Résolution Impossible")
            return -1
        else:
            for j in range(n):
    ...
    if matrice[n - 1][n - 1] == 0:
        ...
        for s in range(n):
            ...
            canvas.create_window((500 * n - 50) / 2, 200 + 35 * s, window=solution[s])
...
def factlu(mat1):
    ...
    if (arret == 0) and (mat[n - 1][n - 1] != 0):
        ...
        messagebox.showerror("Erreur", "Resolution impossible")
        return -1
...
def operation_lu(mat, vect, n):
    ...
    if l != 0:
        ...
        for s in range(len(x)):
            ...
            canvas.create_window((300 * n - 50) / 2 - 40, 20 + 35 * s, window=solution[s])
        canvas.create_text((500 * n - 100) / 2 - 120, 20 + (35 * n / 2) - 20,
                            text="S=",
                            font="/font/myfont 10 ",
                            fill="black"
                        )
        for i in range(n):
            for j in range(n):
                ...
                canvas.create_text(40, 150 * n - 245 + (50 * n / 2) - 20,
                                    text="L = ",
                                    font="/font/myfont 12 ",
                                    fill="#00F5FF"
                                )
                canvas.create_window((300 * n - 100) / 2 + 50 + 60 * j + 10, 150 * n - 245 + 50 * i, window=matrice_u[i][j])
                canvas.create_text((300 * n - 100) / 2, 150 * n - 245 + (50 * n / 2) - 20,
                                    text="U = ",
                                    font="/font/myfont 12 ",
                                    fill="#00F5FF"
                                )
